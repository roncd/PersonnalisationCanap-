    ...
    <main>
        <div class="container">
            ...
            <table class="styled-table">
                ...
                <?php
                    if ($search) {
                        $stmt = $pdo->prepare("SELECT * FROM couleur_tissu WHERE nom LIKE ?");
                        $stmt->execute(['%' . $search . '%']);
                    } else {
                    ...
                    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                        ...
                        echo "<td>{$row['nom']}</td>";
                        echo "<td>{$row['prix']}</td>";
                        echo "<td><img src='../uploads/couleur-tissu-tissu/{$row['img']}' alt='{$row['nom']}' style='width:50px; height:auto;'></td>";
                        echo "<td class='actions'>";
                        echo "<a href='edit.php?id={$row['id']}' class='edit-action actions vert' title='Modifier'><i class='fas fa-edit'></i></a>";
                        echo "<a href='delete.php?id={$row['id']}' class='delete-action actions rouge' title='Supprimer' onclick='return confirm(\"Voulez-vous vraiment supprimer cette couleur de tissu - tissu ?\");'><i class='fas fa-trash-alt'></i></a>";
                        echo "</td>";
                        echo "</tr>";
                    }
                    ?>
