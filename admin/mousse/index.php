        ...
        <div class="tab-container">
            <table class="styled-table">
                ...
                <tbody>
                    <?php
                    if ($search) {
                        $stmt = $pdo->prepare("SELECT * FROM mousse WHERE nom LIKE ?");
                        $stmt->execute(['%' . $search . '%']);
                    } else {
                    ...
                    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                        ...
                        echo "<td>{$row['nom']}</td>";
                        echo "<td>{$row['prix']}</td>";
                        echo "<td><img src='../uploads/mousse/{$row['img']}' alt='{$row['nom']}' style='width:50px; height:auto;'></td>";
                        echo "<td class='actions'>";
                        echo "<a href='edit.php?id={$row['id']}' class='edit-action actions vert' title='Modifier'><i class='fas fa-edit'></i></a>";
                        echo "<a href='delete.php?id={$row['id']}' class='delete-action actions rouge' title='Supprimer' onclick='return confirm(\"Voulez-vous vraiment supprimer cette mousse ?\");'><i class='fas fa-trash-alt'></i></a>";
                        echo "</td>";
                        echo "</tr>";
                    }
                    ?>
