        ...
        <div class="tab-container">
            <table class="styled-table">
                ...
                <tbody>
                    <?php
                    if ($search) {
                        $stmt = $pdo->prepare("SELECT * FROM modele WHERE nom LIKE ?");
                        $stmt->execute(['%' . $search . '%']);
                    } else {
                    ...
                    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                        ...
                        echo "<a href='delete.php?id={$row['id']}' class='delete-action actions rouge' title='Supprimer' onclick='return confirm(\"Voulez-vous vraiment supprimer ce modèle de banquette ?\");'><i class='fas fa-trash-alt'></i></a>";
                        echo "</td>";
                        echo "</tr>";
                    }
                    ?>
